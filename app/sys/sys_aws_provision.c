...
/* Public variables --------------------------------------------------------- */
uint8_t g_aws_provision_state = AWS_NOT_PROVISION_STATE;
...
static bool m_sys_aws_register_thing(char *token, int token_len, const char *device_id);
 ...
 * @param[in]     device_id       Pointer to device ID
 *
 * @attention     None
 *
 * @return
                                                 ...
                                                 void *p_data)
{
  ...
  if (json_obs)
  {
    ...
    {
      char device_id[50];
      memset(device_id, 0, 50);

      // Get device id store in nvs
      // sys_nvs_get_device_id(device_id);
  ...
  if (json_obs)
  {
    ...
    g_aws_provision_state = AWS_PROVISION_DONE;
    // sys_nvs_set_aws_provision_state(g_aws_provision_state);
    ...
    {
      char thing_name[50];
      memset(thing_name, 0, 50);
      strncpy(thing_name, params->payload + json_obs->start, json_obs->end - json_obs->start);
      // sys_nvs_set_thing_name((const char *) thing_name);
