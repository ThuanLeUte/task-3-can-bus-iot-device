...
IoT_Error_t registerJsonTokenOnDelta(jsonStruct_t *pStruct, const char *pShadowName) {
	...
	// if(!deltaTopicSubscribedFlag) {
		ESP_LOGE("TAG", "Thing name: %s", myThingName);
		snprintf(shadowDeltaTopic, MAX_SHADOW_TOPIC_LENGTH_BYTES, "$aws/things/%s/shadow/name/%s/update/delta", myThingName, pShadowName);
		...
		ESP_LOGE("TAG", "Topic register delta: %s", shadowDeltaTopic);
	// }
...
static void shadow_delta_callback(AWS_IoT_Client *pClient, char *topicName,
	...
	for(i = 0; i < tokenTableIndex; i++) {
		...
		if(!tokenTable[i].isFree) {
			...
			if (NULL != strstr(topicName, tokenTable[i].pKey))
			{
